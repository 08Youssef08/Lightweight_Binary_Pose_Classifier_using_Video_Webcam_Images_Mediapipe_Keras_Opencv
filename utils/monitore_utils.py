...
def capture_monitor(data_path,model,source="camera"):
    ...
    # Make Detections
                X = pd.DataFrame([row])
                body_language_prob = model.predict(X)[0]  # This returns the probabilities for each class
...
def image_monitor(data_path,model):
    mp_drawing = mp.solutions.drawing_utils
    mp_holistic = mp.solutions.holistic
    with mp_holistic.Holistic(min_detection_confidence=0.5,min_tracking_confidence=0.5) as holistic:
