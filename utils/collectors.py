...
def image_collector(data_path,output_csv,state="Asleep"):
...
def capture_collector(data_path='/',output_csv='default.csv',source="Camera",state="Asleep"):
    if source == "video":
    ...
    # Loop through the frames of the video
        while cap.isOpened():
            ...
            try:
                ...
                # Create a row of data to write to the CSV
                row = pose_row + face_row
                row.insert(0,state)
                with open(output_csv,mode='a',newline='') as f:
                    csv_writer = csv.writer(f,delimiter=',',quotechar='"',quoting=csv.QUOTE_MINIMAL)
